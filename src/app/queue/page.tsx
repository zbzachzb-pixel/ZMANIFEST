...
export default function QueuePage() {
  ...
  const filterTandem = (student: QueueStudent) => {
    if (!searchTandem) return true
    const search = searchTandem.toLowerCase()
    return (
      student.name.toLowerCase().includes(search) ||
      student.weight.toString().includes(search) ||
      (student.tandemWeightTax && student.tandemWeightTax.toString().includes(search))
    )
  }
  
  const filterAFF = (student: QueueStudent) => {
    if (!searchAFF) return true
    const search = searchAFF.toLowerCase()
    return (
      student.name.toLowerCase().includes(search) ||
      student.weight.toString().includes(search) ||
      (student.affLevel && student.affLevel.toLowerCase().includes(search))
    )
  }
  
  const filteredTandem = tandemQueue.filter(filterTandem)
  const filteredAFF = affQueue.filter(filterAFF)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 p-8">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-white mb-2">Student Queues</h1>
          <p className="text-slate-300">Manage waiting students and create assignments</p>
        </div>
